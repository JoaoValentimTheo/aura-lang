...
### 17.1 Compile-time macros (AuraMacroFactory)

Decorators above are **runtime** macros: they wrap a function and cost a call.
Aura also has **compile-time** macros, expanded by the transpiler *before* any
Python is emitted. A macro sees its operands as quoted AST, returns replacement
AST, and leaves no trace at runtime unless the expansion emits one.
...
def main() {
  ...
  static_assert(true)       // checked while compiling; non-literal is an error
  print(identity(41) + 1)   // expands to `print(41 + 1)`
  print(stringify(42))      // folds the literal to "42" at compile time
  let mut a = 1
  let mut b = 2
  swap(a, b)                // expands to a binding plus two assignments
  print(debug_value(a))     // prints "a = 1", then yields 1
}
```

Built-in compile-time macros: `assert_eq`, `assert_ne`, `static_assert`,
`identity`, `discard`, `stringify`, `swap`, `debug_value`, `todo`, `unreachable`.

A macro that introduces bindings uses **hygienic** names generated by `gensym`,
so it can never capture a local at the call site. A program's own declaration
always wins: `def swap(a, b) { ... }` shadows the `swap` macro.

New macros are registered at build time from Python, using the AST helpers in
`aura.transpiler.macro_factory` (`Quote`, `gensym`, `literal_value`, `name_of`).
The engine is documented in `aura/stdlib/macros.py`.
